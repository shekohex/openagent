import { base64ToUint8Array, base64UrlToUint8Array, CryptoError, generateRandomBytes, uint8ArrayToBase64, uint8ArrayToBase64Url, } from "./crypto";
...
export class KeyExchange {
    ...
    static async generateEphemeralKeyPair() {
        try {
            ...
            throw new CryptoError("Failed to generate ephemeral key pair", error);
        }
    }
    static async deriveSharedSecret(privateKeyData, publicKeyData) {
        try {
            const privateKeyBytes = base64ToUint8Array(privateKeyData);
            const publicKeyBytes = base64UrlToUint8Array(publicKeyData);
            const privateKey = await crypto.subtle.importKey("pkcs8", privateKeyBytes, { name: "ECDH", namedCurve: "P-256" }, false, ["deriveKey"]);
            const publicKey = await crypto.subtle.importKey("raw", publicKeyBytes, { name: "ECDH", namedCurve: "P-256" }, false, []);
    ...
    static async sealMessage(message, recipientPublicKey, senderPrivateKey, recipientKeyId) {
        try {
            const sharedSecret = await KeyExchange.deriveSharedSecret(senderPrivateKey, recipientPublicKey);
            const iv = generateRandomBytes(KeyExchange.NONCE_LENGTH);
            const plaintextBytes = new TextEncoder().encode(message);
            const ciphertext = await crypto.subtle.encrypt({ name: "AES-GCM", iv: iv }, sharedSecret, plaintextBytes);
            ...
            throw new CryptoError("Failed to seal message", error);
        }
    }
    static async openMessage(payload, recipientPrivateKey, senderPublicKey) {
        try {
            if (!KeyExchange.validateSealedPayload(payload)) {
                throw new CryptoError("Invalid sealed payload");
            }
            const sharedSecret = await KeyExchange.deriveSharedSecret(recipientPrivateKey, senderPublicKey);
            ...
            const decrypted = await crypto.subtle.decrypt({ name: "AES-GCM", iv: nonceBytes }, sharedSecret, combinedCiphertext);
            return new TextDecoder().decode(decrypted);
        }
        catch (error) {
            throw new CryptoError("Failed to open sealed message", error);
        }
    }
    ...
    static validateSealedPayload(payload) {
        if (typeof payload !== "object" || payload === null) {
            return false;
        }
        const p = payload;
        return (typeof p.ciphertext === "string" &&
...
export class SecureProviderKeyDelivery {
    static async packageProviderKeys(providerKeys, recipientPublicKey, senderPrivateKey, recipientKeyId) {
        const payload = {
        ...
        return await KeyExchange.sealMessage(message, recipientPublicKey, senderPrivateKey, recipientKeyId);
    }
    static async unpackProviderKeys(sealedPayload, recipientPrivateKey, senderPublicKey) {
        const message = await KeyExchange.openMessage(sealedPayload, recipientPrivateKey, senderPublicKey);
